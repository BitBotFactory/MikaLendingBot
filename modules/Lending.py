...
defaults = dict(sleep_time_active=0, sleep_time_inactive=0, sleep_time=0, min_daily_rate=0, max_daily_rate=0,
                spread_lend=0,
                gap_bottom_default=0, gap_top_default=0, xday_threshold=0, xday_spread=0, xdays=0, min_loan_size=0,
                min_loan_sizes={}, end_date=None, coin_cfg={}, dry_run=0, transferable_currencies=[],
                currencies_to_analyse=[], keep_stuck_orders=True, hide_coins=True, coin_cfg_alerted={},
                max_active_alerted={}, notify_conf={}, loans_provided={}, gap_mode_default="", scheduler=None,
                exchange=None, frrasmin=False, frrdelta=0.0)
...
class Lending:
    def __init__(self, cfg, api1, log1, data, maxtolend, dry_run1, analysis, notify_conf1):
        ...
        conf = dict(sleep_time_active=float(Config.get("BOT", "sleeptimeactive", None, 1, 3600)),
                    ...
                    end_date=Config.get('BOT', 'endDate'), coin_cfg=Config.get_coin_cfg(),
                    min_loan_sizes=Config.get_min_loan_sizes(), dry_run=dry_run1,
        ...
        if conf['analysis_method'] not in ['percentile', 'MACD']:
            raise ValueError(
                "analysis_method: \"{0}\" is not valid, must be percentile or MACD".format(conf['analysis_method']))
        ...
        if notify_conf['notify_summary_minutes']:
            # Wait 10 seconds before firing the first summary notifcation, then use config time value for future updates
    ...
    def set_sleep_time(self, usable):
        global sleep_time
    ...
    def notify_summary(self, sleep_time):
        try:
    ...
    def notify_new_loans(self, sleep_time):
    ...
    def get_frr_or_min_daily_rate(self, cur):
        """
    ...
    def get_min_daily_rate(self, cur):
    ...
    def construct_order_book(self, active_cur):
    ...
    def get_gap_rate(self, active_cur, gap, order_book, cur_total_balance, raw=False):
        if raw:
            gap_expected = gap
        else:
    ...
    def get_cur_spread(self, spread, cur_active_bal, active_cur):
    ...
    def construct_orders(self, cur, cur_active_bal, cur_total_balance, ticker):
    ...
    def get_gap_mode_rates(self, cur, cur_active_bal, cur_total_balance, ticker):
    ...
    def lend_cur(self, active_cur, total_lent, lending_balances, ticker):
        construct_order_book = self.construct_order_book
        ...
        active_bal = MaxToLend.amount_to_lend(active_cur_total_balance, active_cur,
                                              Decimal(lending_balances[active_cur]),
                                              Decimal(order_book['rates'][0]))

        if float(active_bal) >= get_min_loan_size(
                active_cur):  # Make sure sleeptimer is set to active if any cur can lend.
            currency_usable = 1
        else:
            return 0  # Return early to end function.

        orders = construct_orders(active_cur, active_bal, active_cur_total_balance,
                                  ticker)  # Build all potential orders
        i = 0
