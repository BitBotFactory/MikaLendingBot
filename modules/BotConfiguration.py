...
class BotConfig:
    ...
    def __init__(self, file_location):
        ...
        if len(loaded_files) != 1:
            ...
            try:
                shutil.copy('settings/default.cfg.example', file_location)
                ...
                ex.m = str(ex)
                print("Failed to automatically copy config. Please do so manually. Error: {0}".format(ex.m))
                exit(1)
    ...
    def getboolean(self, category, option, default_value=False):
        if self.has_option(category, option):
            try:
                ...
                return self.config.getboolean(category, option)
        else:
    ...
    def get(self, category, option, default_value=False, lower_limit=False, upper_limit=False):
        if self.has_option(category, option):
            try:
                ...
                value = self.config.get(category, option)
            try:
    ...
    def get_gap_mode(self, category, option='gapmode'):
